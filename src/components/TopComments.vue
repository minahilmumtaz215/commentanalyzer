          ...
          <td>
            ...
            <div v-if="comment.replies?.length" class="replies-section">
              <div class="replies-heading" @click="toggleReplies(index)">
                Replies ({{ Math.min(comment.replies.length, 5) }})
              </div>

              <div v-if="expandedReplies[index]" class="replies-container">
                <div
                  v-for="(reply, rIndex) in comment.replies.slice(0, 5)"
                  :key="rIndex"
                  class="reply"
                >
                  <div class="reply-number">{{ rIndex + 1 }}.</div>
                  <div class="reply-text">{{ reply.text }}</div>
                  <div class="reply-likes">👍 {{ reply.likes }}</div>
                </div>
              </div>
            </div>
          </td>
          <td class="likes">👍 {{ comment.likes }}</td>
        </tr>
      </tbody>
    </table>
  </div>
